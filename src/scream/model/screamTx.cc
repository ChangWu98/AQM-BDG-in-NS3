...
namespace ns3
{
NS_LOG_COMPONENT_DEFINE("ScreamTx");
...
static const uint64_t kMinRtpQueueDiscardInterval_us = 500000;
...
ScreamTx::ScreamTx(float lossBeta_,
    ...
    rateAcked(0.0f),
    m_bytesReceived(0),
    m_sumBytesReceived(0),
    m_lastCalRecRate(10100),
    //m_lastCalRecRate(10),
    ...
    //fp_log(fopen("/home/ubuntu-ns3/ns-allinone-3.35/ns-3.35/output/ScreamFileLog.txt", "w+")),
    fp_log(0),
...
float ScreamTx::isOkToTransmit(uint64_t time_us, uint32_t &ssrc) {
    ...
    */
    ...
    if (kEnablePacketPacing && nextTransmitT_us > time_us){
        retVal = (nextTransmitT_us - time_us);
    }
...
float ScreamTx::addTransmitted(uint64_t time_us,
    ...
    int id;
    if(1e-6*time_us>=startRecordTime && 1e-6*time_us<=stopRecordTime)
        accBytesTransmitted+=(size-12);
    Stream *stream = getStream(ssrc,id);
    ...
    //     NS_LOG_UNCOND("In addTransmitted : stream->targetBitrate = stream->minBitrate;");
    // }
...
void ScreamTx::incomingFeedback(uint64_t time_us,
    ...
    */
    ...
    if(timestamp >= m_lastCalRecRate + 40){
        float recRate = (float)m_bytesReceived*8*1e-6 / (1e-3*(timestamp - m_lastCalRecRate));//Mbps
        if(!m_throughputCb.IsNull()){m_throughputCb((float)((timestamp*1e-3) - 9.9),recRate);}
        //if(!m_throughputCb.IsNull()){m_throughputCb((float)(time_us*1e-6),recRate);}
        m_bytesReceived = 0;
        m_lastCalRecRate = int64_t(timestamp);
        if(time_us > 19900000){cout<<"sumBytesReceived: "<<m_sumBytesReceived<<endl;}
    }

    if(!lossEvent) isLosing=false;
        ...
        if (txPackets[n].isUsed) {
            ...
            */
            if (seqNrExt <= highestSeqNrExt && tmp->isAfterReceivedEdge == false) {
                ...
                //m_bytesReceived = m_bytesReceived + size + 12;
                if(time_us > 10000000){
                    m_bytesReceived = m_bytesReceived + tmp->size;
                    m_sumBytesReceived = m_sumBytesReceived + tmp->size;
                }
            }
...
uint64_t ScreamTx::getByteTranStatistics(){
    return accBytesTransmitted;
}
void ScreamTx::getByteTranClear(){
    accBytesTransmitted=0;
}

uint64_t ScreamTx::getByteReceiveStatistics(){
    return accBytesAcked;
}
void ScreamTx::getByteReceiveClear(){
    accBytesAcked=0;
}

int ScreamTx::getNumLossEvent(){
    return numLossEvent;
}

int ScreamTx::getNumLossPacket(){
    return numLossPacket;
}
...
void ScreamTx::updateCwnd(uint64_t time_us) {
    ...
    if ((time_us - lastAddToQueueDelayFractionHistT_us) >
        ...
        */
        ...
        if ((queueDelayFractionAvg > 0.02f || isL4s) && kEnablePacketPacing) {
            float headRoom = kPacketPacingHeadRoom;
            if (lastCongestionDetectedT_us == 0) {
				/*
            ...
            */
            ...
            float pacingBitrate = (1.0 - cautiousPacing)*cwnd * 8.0f / std::max(0.00001f, sRtt) + cautiousPacing*rateTransmittedAvg;
            pacingBitrate = std::max(kMinimumBandwidth, headRoom*pacingBitrate);
            float tp = (mss * 8.0f) / pacingBitrate;//tp(s)
            paceInterval = std::max(kMinPaceInterval, tp);//paceInterval(s)
        }
        paceInterval_us = (uint64_t)(paceInterval * 1000000);
        //cout<<"paceInterval_us: "<< paceInterval_us<<endl;
    ...
    */
    ...
    if (lossEvent) {
        ...
        */
        ...
        inFastStart = false;
        wasLossEvent = true;
        NS_LOG_UNCOND(time_us*1e-6);
        NS_LOG_UNCOND("inFastStart sets to false for Los");
    }
    else if (ecnCeEvent) {
        /*
        * loss event detected, decrease congestion window
        */
        if (isL4s) {
            cwnd = std::max(cwndMin, (int)((1.0f - l4sAlpha / (2.0f*kPacketPacingHeadRoom))*cwnd));
        }
        else {
        ...
        wasLossEvent = true;
        NS_LOG_UNCOND(time_us*1e-6);
        NS_LOG_UNCOND("inFastStart sets to false for ECN");
    }
    else {
            ...
            */
            if (queueDelayTrend < 0.2) {
                ...
                */
                ...
                inFastStart = false;
                NS_LOG_UNCOND(time_us*1e-6);
                NS_LOG_UNCOND("inFastStart sets to false for queueDelayTrend > 0.2");
            }
        }
        else {
            if (offTarget > 0.0f) {
    ...
    */
    if (maxBytesInFlight > 5000 && lastCongestionDetectedT_us > 0) {
    ...
    */
    if (queueDelayTrend > 0.2) {
        ...
        */
        ...
        lastCongestionDetectedT_us = time_us;
        NS_LOG_UNCOND(time_us*1e-6);
        NS_LOG_UNCOND("inFastStart resets to true");
    }
    bytesNewlyAcked = 0;
...
float ScreamTx::Stream::getMaxRate() {
    return std::max(rateTransmitted, rateAcked);//by Ericsson
...
void ScreamTx::Stream::updateTargetBitrate(uint64_t time_us) {
    ...
    */
    ...
    if (lossEventFlag || ecnCeEventFlag) {
        ...
        */
        //if (time_us - lastTargetBitrateIUpdateT_us > 1000000) {
            /*
        ...
        */
        ...
        else if (parent->inFastStart && rtpQueueDelay < 0.2f) {
        //else if (parent->inFastStart && rtpQueueDelay < 0.02f) {
            /*
            ...
            */
            ...
            if (wasFastStart) {
                wasFastStart = false;
                if (time_us - lastTargetBitrateIUpdateT_us > 500000) {
                    /*
                    * The timing constraint avoids that targetBitrateI
            ...
            */
            ...
            if (increment > 0) {
				 ...
				 *  the bitrate up some extra
				 */
                //float incrementScale = 1.0f + 0.05f*std::min(1.0f, 250000.0f / targetBitrate);
				float incrementScale = 1.0f + 0.05f*std::min(1.0f, 10e6f / targetBitrate);
