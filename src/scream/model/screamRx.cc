...
void ScreamRx::Stream::receive(uint64_t time_us,
    ...
    */
    ...
    /*
    * Update the ACK vector that indicates reception '=1' of RTP packets prior to
    * the highest received sequence number. ACK矢量，表示在最高seqnr(时间)之前接收的RTP数据包。
    * The next highest SN is indicated by the least significant bit,(最低有效位)
    ...
    * 64 RTP packets, 一个64位的ACK矢量意味着它理论上可以每64个RTP包发送一个反馈while this can possibly work at low bitrates, it is less likely to work
    ...
    */
    ...
    receiveTimestamp = (uint32_t)(time_us / 1000);//receiveTimestamp default clock frequency = 1000Hz  (1ms)
...
bool ScreamRx::getFeedback(uint64_t time_us,
    ...
    for (auto it = streams.begin(); it != streams.end(); ++it) {
        if ((*it)->nRtpSinceLastRtcp > 0 && (*it)->lastFeedbackT_us < minT_us) {
        	//选出有新RTP包接收 且 上次反馈RTCP包最早（距此时最久）的那个Stream
            stream = *it;
...
bool ScreamRx::createFeedback(uint64_t time_us, unsigned char *buf, int &size) {
    ...
    if (getFeedback(time_us, ssrc_src, timeStamp, seqNr, ackVector, ecnCeMarkedBytes)) {
        ...
        //为什么右移32位?Answer:取64位ackVector的高32位
        tmp_l = htonl(tmp_l);
        memcpy(buf + 20, &tmp_l, 4);
        tmp_l = uint32_t(ackVector & 0xFFFFFFFF);//取低32位
        tmp_l = htonl(tmp_l);
