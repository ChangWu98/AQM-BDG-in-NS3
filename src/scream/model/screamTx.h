...
static const float kQueueDelayTargetMin = 0.05f; //ms
...
static const float kRampUpSpeed = 1000000.0f; // bps/s
...
*/
static const int kMaxStreams = 40;
/*
* History vectors
*/
...
public:
    ...
    */
    ScreamTx(float lossBeta = kLossBeta,
        ...
        int cwnd = 20000,  // An initial cwnd larger than 2*mss
        float cautiousPacing = 0.1f,
        int bytesInFlightHistSize = 20,
        bool isL4s = false,
    ...
    * >0.0 : Time [us] until this function should be called again
    ...
    */
    ...
    uint64_t getByteTranStatistics();
    void getByteTranClear();
	void getByteReceiveClear();
    uint64_t getByteReceiveStatistics();
    int getNumLossEvent();
    int getNumLossPacket();

    typedef Callback<void,float,float> ThroughputCallback;
	void SetThroughputCallback(ThroughputCallback cb){m_throughputCb=cb;}



private:
    ThroughputCallback m_throughputCb;
    /*
    ...
    */
    int bytesNewlyAcked;
    uint64_t accBytesAcked;
    uint64_t accBytesTransmitted;
    float startRecordTime;
    float stopRecordTime;
    ...
    float rateAcked;
    uint64_t m_bytesReceived;//byte
    int64_t m_lastCalRecRate;//ms
    uint64_t m_sumBytesReceived;
