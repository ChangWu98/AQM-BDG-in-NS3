...
static const int kMaxRtpSize = 1500;
...
int VideoEnc::encode(float time) {
    int rtpBytes = 0;
     int bytes=0;
    //bytes=std::min(int(1.1*frameSize[ix]),(int)ceil(0.001*m_fs*targetBitrate/8));
    //bytes=ceil(0.001*m_fs*min(targetBitrate,nominalBitrate)/8);
    //bytes = (int) (frameSize[ix]/nominalBitrate*targetBitrate);
    cout<<"Time: "<<time<<endl;
    bytes=ceil(0.001*m_fs*targetBitrate/8);
